...
export const FundsPanel: React.FC<FundsPanelProps> = ({ 
  ...
  const renderHeaderAction = (
    <div className="flex items-center gap-4">
        {/* Only show tabs if expanded */}
        {isExpanded && (
            <div className="hidden sm:flex bg-slate-900 rounded-lg p-1 overflow-x-auto">
                {tabs.map(tab => (
                    <button
                    key={tab.id}
  ...
  return (
    <Card title="Depósitos y Retiros" action={renderHeaderAction}>
        {/* Collapsible Content */}
        {isExpanded ? (
            <div className="space-y-6 animate-in fade-in slide-in-from-top-4 duration-300">
                
                {/* Mobile Tabs (only visible when expanded on small screens) */}
                <div className="sm:hidden flex bg-slate-900 rounded-lg p-1 overflow-x-auto mb-4">
                    {tabs.map(tab => (
                        <button
                        key={tab.id}
                        onClick={() => setViewMode(tab.id)}
